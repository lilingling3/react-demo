...
//var API_HOST  = process.env.API_HOST ? process.env.API_HOST : 'http://pmptest.boldseas.com';
//var API_HOST  = process.env.API_HOST ? process.env.API_HOST : 'http://192.168.5.3:2088';
var API_HOST  = process.env.API_HOST ? process.env.API_HOST : 'http://localhost:3000';
console.log('the API_HOST: ' + API_HOST);

var DT = process.env.DT ? 'true' : 'false';
var DEVICE= process.env.DEVICE ? process.env.DEVICE : '';

var srcRootDirName = process.env.ROOT_DIR ? process.env.ROOT_DIR : 'src';
console.log('the ROOT_DIR: ' + srcRootDirName);
...
var webpackConfig = {
  ...
  entry: [
    ...
    path.join(__dirname, '../'+srcRootDirName+'/main.js')
  ],
  output: {
  ...
  module: {
    loaders: [
      {test: /\.js$/, loaders: ['react-hot', 'babel'], include: path.join(__dirname, '../'+srcRootDirName)},
      {test: /\.(woff|svg|eot|ttf)\??.*$/, loader: 'url-loader?limit=1&name=[path][name].[ext]'},
      {test: /\.css$/, loaders: ['style', 'raw'], include: path.join(__dirname, '../'+srcRootDirName)}
    ]
  },
  plugins: [
